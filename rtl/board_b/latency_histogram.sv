...
(
    ...
    // B3: most recent latency sample (latched on each fill_processed)
    output logic [COUNTER_W-1:0]  last_latency
);
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // B3: latch the most recent latency sample for live ticker display
            last_latency <= {16'b0, latency};
        end
    end
