# ============================================================================
# create_board_a.tcl — Vivado project + block design for HFT Board A
# ============================================================================
...
add_files -norecurse [glob "[file join $proj_root rtl board_a]/*.sv"]
add_files -norecurse [list [file normalize [file join $proj_root rtl board_a board_a_top_bd.v]]]
...
create_bd_cell -type module -reference board_a_top_bd hft_core

# ── 7. Add infrastructure IPs ─────────────────────────────────────────────
# SmartConnect bridges the PS AXI4 master to our AXI-Lite slave.
...
# Processor System Reset synchronises the async PS resetn to the PL clock.
...
# ── 10. Connect AXI ───────────────────────────────────────────────────────
# PS master → SmartConnect slave
connect_bd_intf_net \
    [get_bd_intf_pins zynq_ps/M_AXI_HPM0_FPD] \
    [get_bd_intf_pins axi_sc/S00_AXI]

# SmartConnect master → board_a_top AXI-Lite slave
connect_bd_intf_net \
    [get_bd_intf_pins axi_sc/M00_AXI] \
    [get_bd_intf_pins hft_core/s_axi]

# ── 11. Create external I/O ports ─────────────────────────────────────────
# Names MUST match the XDC exactly (constraints/hft_top.xdc).

# Inputs
create_bd_port -dir I -from 3 -to 0 btn
create_bd_port -dir I -from 7 -to 0 sw
create_bd_port -dir I              pmod_ja_ready
create_bd_port -dir I -from 3 -to 0 pmod_jb
create_bd_port -dir I              pmod_jb_valid

# Outputs
create_bd_port -dir O -from 7 -to 0 led
create_bd_port -dir O -from 2 -to 0 rgb0
create_bd_port -dir O -from 2 -to 0 rgb1
create_bd_port -dir O -from 3 -to 0 pmod_ja
create_bd_port -dir O              pmod_ja_valid
create_bd_port -dir O              pmod_jb_ready
...
connect_bd_net [get_bd_ports pmod_ja_ready]  [get_bd_pins hft_core/pmod_ja_ready]
connect_bd_net [get_bd_ports pmod_jb]        [get_bd_pins hft_core/pmod_jb]
connect_bd_net [get_bd_ports pmod_jb_valid]  [get_bd_pins hft_core/pmod_jb_valid]
...
connect_bd_net [get_bd_pins hft_core/pmod_ja]       [get_bd_ports pmod_ja]
connect_bd_net [get_bd_pins hft_core/pmod_ja_valid] [get_bd_ports pmod_ja_valid]
...
puts "   See design_specification.md Appendix D.1"
puts ""
puts " Next steps:"
