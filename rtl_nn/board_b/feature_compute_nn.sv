// ============================================================================
// Module: feature_compute_nn
// Identical to feature_compute with two additional output ports:
//   mid_delta — current mid minus previous mid (signed, Q16.16)
//   ema_delta — current ema minus previous ema (signed, Q16.16)
// These are needed by nn_inference for features 2 and 3.
...
#(
    ...
    // ── NEW: delta outputs for nn_inference ─────────────────────
    output sprice_t     mid_delta,   // mid[t] - mid[t-1], signed Q16.16
    output sprice_t     ema_delta    // ema[t] - ema[t-1], signed Q16.16
);

    // ── Per-symbol EMA state ────────────────────────────────────
    price_t  ema_state [NUM_SYM];
    logic    ema_init  [NUM_SYM];

    // ── Per-symbol previous mid/ema for delta computation ───────
    price_t  mid_prev [NUM_SYM];   // NEW
    price_t  ema_prev [NUM_SYM];   // NEW
    ...
    logic [47:0] s2_prod_a;
    logic [47:0] s2_prod_b;
    price_t      s2_mid;
    price_t      s2_bid, s2_ask;
    symbol_t     s2_sym;
    logic        s2_valid;
    logic        s2_first;
    price_t      s2_mid_prev;   // NEW: carry prev mid through pipeline
    price_t      s2_ema_prev;   // NEW: carry prev ema through pipeline
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (s1_valid) begin
                ...
                s2_ask <= s1_ask;
                s2_sym <= s1_sym;
                // NEW: latch previous values for this symbol
    ...
    price_t spread_s2;
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) spread_s2 <= '0;
        else        spread_s2 <= s1_spread;
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (s2_valid) begin
                ...
                // NEW: compute deltas from previous values
                mid_delta <= $signed({1'b0, s2_mid}) - $signed({1'b0, s2_mid_prev});

                if (s2_first) begin
                    ema               <= s2_mid;
                    deviation         <= '0;
                    ema_delta         <= '0;  // NEW: no delta on first sample
                    ema_state[s2_sym] <= s2_mid;
                    ema_init[s2_sym]  <= 1'b1;
                    mid_prev[s2_sym]  <= s2_mid;   // NEW
                    ema_prev[s2_sym]  <= s2_mid;   // NEW
                end else begin
                    logic [47:0] sum48;
                    price_t      ema_new;
