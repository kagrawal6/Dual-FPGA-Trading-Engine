...
module tb_exchange_lite();
    ...
    initial begin
        ...
        if (fill_frame[127:124] !== MSG_FILL) begin
            $display("FAIL: T1 msg_type=%h, expected FILL", fill_frame[127:124]);
            err_cnt = err_cnt + 1;
        end else if (fill_frame[114:112] !== 3'b000) begin
            $display("FAIL: T1 status=%b, expected FILLED(000)", fill_frame[114:112]);
            err_cnt = err_cnt + 1;
        end else if (fill_frame[111:80] !== 32'h0065_0000) begin
            $display("FAIL: T1 fill_price=%h, expected ask=0065_0000", fill_frame[111:80]);
            err_cnt = err_cnt + 1;
        end else if (fill_frame[79:64] !== 16'd100) begin
            $display("FAIL: T1 fill_qty=%0d, expected 100", fill_frame[79:64]);
            err_cnt = err_cnt + 1;
        end else
            $display("PASS: T1 BUY filled at ask price, qty=100");
        ...
        if (fill_frame[114:112] !== 3'b001) begin
            $display("FAIL: T2 status=%b, expected REJECTED(001)", fill_frame[114:112]);
            err_cnt = err_cnt + 1;
        end else
            $display("PASS: T2 BUY below ask rejected");
        ...
        if (fill_frame[114:112] !== 3'b000) begin
            $display("FAIL: T3 status=%b, expected FILLED", fill_frame[114:112]);
            err_cnt = err_cnt + 1;
        end else if (fill_frame[115] !== 1'b1) begin
            $display("FAIL: T3 side=%b, expected SELL(1)", fill_frame[115]);
            err_cnt = err_cnt + 1;
        end else if (fill_frame[111:80] !== 32'h0064_0000) begin
            $display("FAIL: T3 fill_price=%h, expected bid=0064_0000", fill_frame[111:80]);
            err_cnt = err_cnt + 1;
        end else
            $display("PASS: T3 SELL filled at bid price");
        ...
        if (fill_frame[63:48] !== 16'hABCD) begin
            $display("FAIL: T4 order_id=%h, expected ABCD", fill_frame[63:48]);
            err_cnt = err_cnt + 1;
        end else if (fill_frame[47:32] !== 16'h1234) begin
            $display("FAIL: T4 timestamp=%h, expected 1234", fill_frame[47:32]);
            err_cnt = err_cnt + 1;
        end else
            $display("PASS: T4 order_id and timestamp echoed correctly");
        ...
        if (orders_rcvd !== 32'd4) begin
            $display("FAIL: T5 orders_rcvd=%0d, expected 4", orders_rcvd);
            err_cnt = err_cnt + 1;
        end else
            $display("PASS: T5 orders_rcvd=4");

        if (fills_sent !== 32'd3) begin
            $display("FAIL: T5 fills_sent=%0d, expected 3", fills_sent);
            err_cnt = err_cnt + 1;
        end else
            $display("PASS: T5 fills_sent=3");

        if (rejects_sent !== 32'd1) begin
            $display("FAIL: T5 rejects_sent=%0d, expected 1", rejects_sent);
            err_cnt = err_cnt + 1;
        end else
            $display("PASS: T5 rejects_sent=1");
